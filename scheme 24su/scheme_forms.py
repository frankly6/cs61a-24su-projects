...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.bindings[first] = scheme_eval(rest.first,env)
        return first
        # END PROBLEM 4
        ...
        lp = do_lambda_form(Pair(formals,body),env)
        env.bindings[name] = lp
        return name
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    vis = nil
    while bindings != nil:
        u = bindings.first.first
        v = bindings.first.rest
        validate_form(v,1,1)
        vis = Pair(u,vis)
        names = Pair(u,names)
        if bindings.rest==nil: vals = Pair(scheme_eval(v.first,env),vals)
        else: vals = Pair(scheme_eval(v.first,env),vals)
        bindings = bindings.rest
    validate_formals(vis)
    # END PROBLEM 14
