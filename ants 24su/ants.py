...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if place.ant.is_container == True and self.is_container == False and place.ant.can_contain(self) == True:
                place.ant.store_ant(self)
            elif place.ant.is_container == False and self.is_container == True and self.can_contain(place.ant) == True:
                self.store_ant(place.ant)
                place.ant = self
            else: assert place.ant is None, 'Too many ants in {0}'.format(place)
            # END Problem 8b
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        np = self.place
        counter = 0
        while counter < self.lower_bound and np.entrance != None: 
            np = np.entrance
            counter += 1
        while ((np.entrance != None) and (random_bee(np.bees) == None)) and counter <= self.upper_bound:
            np = np.entrance
            counter += 1
        if np.entrance == None or counter > self.upper_bound:
            return None
        else: 
            return random_bee(np.bees)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if bee and self.turns_to_chew == 0:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if other.is_container == False and self.ant_contained == None: return True
        else: return False 
        # END Problem 8a
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if not hasattr(target,'left'):
            f = target.action
            def new_act(gamestate):
                if target.left > 0 and gamestate.time%2 == 1: pass
                else: f(gamestate)
                if target.left > 0: 
                    target.left -= 1
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if target == None: return
        if not hasattr(target,'scare_round'):
            f2 = target.action
            def new_act2(gamestate):
                old_place = target.place
                if target.scare_round > 0:
                    target.dir_swap = True
                else:
                    target.dir_swap = False
                f2(gamestate)
                new_place = target.place 
                if target.scare_round > 0 and old_place != new_place: 
                    target.scare_round -= 1
            target.scare_round = 2
            target.action = new_act2    
        # END Problem EC 2
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        for bees in self.place.bees[:]:
            Insect.reduce_health(bees, self.damage)
        # END Problem EC 3
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem EC 4    
        dic = {}
        if self.place.ant != self: dic[self.place.ant]=0
        np, d = self.place, 0
        for bees in np.bees[:]:
            dic[bees]=0
        while np.entrance.is_hive == False:
            np, d = np.entrance, d+1
            if np.ant: 
                dic[np.ant]=d
                if np.ant.is_container == True:
                    if np.ant.ant_contained != None:
                        dic[np.ant.ant_contained]=d
            for bees in np.bees[:]:
                dic[bees]=d
        return dic
        # END Problem EC 4
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        return max(0,2-self.insects_shot/16-distance/4)
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.dir_swap == False: destination = self.place.exit
        if self.dir_swap == True: destination = self.place.entrance

        if self.blocked():
            self.sting(self.place.ant)
        elif self.health > 0 and destination is not None and destination.is_hive == False:
    ...
    def scare(self, length):
        ...
        "*** YOUR CODE HERE ***"
        pass
        # I DIDNT KNOW HOW TO WRITE THIS PART, I JUST OVERRIDE THE ACTION() OF THE BEE
        # I ALSO WRITE ALOTS CODE EVERYWHERE, LIKE A BEE().__INIT__ AND STH., IT WORKED QUITE WELL I GUESS
        # END Problem EC 2
